# PRUEBA DE COMPORTAMIENTO DEL FUNCTIONAL BOXPLOT PARA LA PRIMERA DERIVADA
...
fmboxplot_i <- function(X, t, rangeval, nbasis, norder) {
  ...
  results<-ind(X, t, rangeval, nbasis, norder)
  indexes<-results$ind
  sdata<-results$sddata
  n<-nrow(indexes)
  hypo<-sort(indexes$ddtaMHI)
  epi<-sort(indexes$ddtaMEI)
  #75% con mayor Hypograph
  q_h<-hypo[round(n/4)+1]
  sorted_h <- as.data.frame(subset(sdata,indexes$ddtaMHI>=q_h))
  #75% con menor Epigraph
  q_e<-epi[round(n*3/4)+1]
  sorted_e <- as.data.frame(subset(sdata,indexes$ddtaMEI<=q_e))
  #Bucle para crear q1 y q3
  q1<-rep(0,length(t))
