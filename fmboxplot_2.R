...
param <- function(X, t, rangeval, nbasis, norder) {
  ...
  sddata<-results$sddata
  sd2data<-results$sd2data
  hypo_d<-sort(indexes$ddtaMHI)
  epi_d<-sort(indexes$ddtaMEI)
  hypo_d2<-sort(indexes$d2dtaMHI)
  epi_d2<-sort(indexes$d2dtaMEI)
  ...
  #75% con mayor Hypograph
  q_h_d<-hypo_d[round(n/4)+1]
  q_h_d2<-hypo_d2[round(n/4)+1]
  sorted_h_d<-list()
  sorted_h_d2<-list()
  for (i in 1:dim(X)[3]) {
    sorted_h_d[[i]] <- as.data.frame(subset(sddata[,,i],indexes$ddtaMHI>=q_h_d))
    sorted_h_d2[[i]] <- as.data.frame(subset(sd2data[,,i],indexes$d2dtaMHI>=q_h_d2))
  }
  #75% con menor Epigraph
  q_e_d<-epi_d[round(n*3/4)+1]
  q_e_d2<-epi_d2[round(n*3/4)+1]
  sorted_e_d<-list()
  sorted_e_d2<-list()
  for (i in 1:dim(X)[3]) {
    sorted_e_d[[i]] <- as.data.frame(subset(sddata[,,i],indexes$ddtaMEI<=q_e_d))
    sorted_e_d2[[i]] <- as.data.frame(subset(sd2data[,,i],indexes$d2dtaMEI<=q_e_d2))
  }
  ...
  #Bucle para crear q1 y q3
  q1_d<-array(dim = c(dim(X)[3],length(t)))
  for (j in 1:dim(X)[3]) {
    data<-as.data.frame(sorted_h_d[j])
    for (i in 1:ncol(data)) {
      q1_d[j,i]<-min(data[,i])
    }
  }
  q1_d2<-array(dim = c(dim(X)[3],length(t)))
  for (j in 1:dim(X)[3]) {
    data<-as.data.frame(sorted_h_d2[j])
    for (i in 1:ncol(data)) {
      q1_d2[j,i]<-min(data[,i])
    }
  }
  q3_d<-array(dim = c(dim(X)[3],length(t)))
  for (j in 1:dim(X)[3]) {
    data<-as.data.frame(sorted_e_d[j])
    for (i in 1:ncol(data)) {
      q3_d[j,i]<-max(data[,i])
    }
  }
  q3_d2<-array(dim = c(dim(X)[3],length(t)))
  for (j in 1:dim(X)[3]) {
    data<-as.data.frame(sorted_e_d2[j])
    for (i in 1:ncol(data)) {
      q3_d2[j,i]<-max(data[,i])
    }
  }
  
  #IQR: q3-q1
  IQR_d<-array(dim = c(dim(X)[3],length(t)))
  IQR_d <- q3_d-q1_d
  IQR_d2<-array(dim = c(dim(X)[3],length(t)))
  IQR_d2 <- q3_d2-q1_d2
  ...
  #Lower whisker 
  w1_d <- q1_d - 0.8*IQR_d
  w1_d2 <- q1_d2 - 0.8*IQR_d2
  #Upper whisker 
  w3_d <- q3_d + 0.8*IQR_d
  w3_d2 <- q3_d2 + 0.8*IQR_d2
  ...
  for (i in 1:dim(X)[3]) {
    ...
    w3_<-w3_d[i,]
    for (k in 1:nrow(d)) {
      if((sum(d[k,]>w3_|d[k,]<w1_)>0)&(!is.element(k, outliers_d))){outliers_d<-c(outliers_d,k)}
    }
  }
  outliers_d<-sort(outliers_d)
  
  #Ponemos en común los outliers
  for (i in outliers_d){if(!is.element(i,outliers)){outliers<-c(outliers,i)}}
  ...
  for (i in 1:dim(X)[3]) {
    ...
    w3_<-w3_d2[i,]
    for (k in 1:nrow(d)) {
      if((sum(d[k,]>w3_|d[k,]<w1_)>0)&(!is.element(k, outliers_d2))){outliers_d2<-c(outliers_d2,k)}
    }
  }
  outliers_d2<-sort(outliers_d2)
  
  #Ponemos en común los outliers
  for (i in outliers_d2){if(!is.element(i,outliers)){outliers<-c(outliers,i)}}
  
  
  res<-list("q1"=q1,"q3"=q3,"w1"=w1,"w3"=w3,"sdata"=sdata,"outliers"=outliers,"q1_d"=q1_d,"q3_d"=q3_d,"w1_d"=w1_d,"w3_d"=w3_d,"sddata"=sddata,"q1_d2"=q1_d2,"q3_d2"=q3_d2,"w1_d2"=w1_d2,"w3_d2"=w3_d2,"sd2data"=sd2data)
  return(res)
}
