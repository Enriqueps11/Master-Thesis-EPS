# PRUEBA DE COMPORTAMIENTO DEL FUNCTIONAL BOXPLOT PARA LA SEGUNDA DERIVADA
...
param <- function(X, t, rangeval, nbasis, norder) {
  ...
  indexes<-results$ind
  n<-nrow(indexes)
  sddata<-results$sd2data
  hypo_d<-sort(indexes$d2dtaMHI)
  epi_d<-sort(indexes$d2dtaMEI)
  
  #75% con mayor Hypograph
  q_h_d<-hypo_d[round(n/4)+1]
  sorted_h_d<-list()
  for (i in 1:dim(X)[3]) {
    sorted_h_d[[i]] <- as.data.frame(subset(sddata[,,i],indexes$d2dtaMHI>=q_h_d))
  }
  #75% con menor Epigraph
  q_e_d<-epi_d[round(n*3/4)+1]
  sorted_e_d<-list()
  for (i in 1:dim(X)[3]) {
    sorted_e_d[[i]] <- as.data.frame(subset(sddata[,,i],indexes$d2dtaMEI<=q_e_d))
  }
  ...
  res<-list("outliers"=outliers_d,"q1"=q1_d,"q3"=q3_d,"w1"=w1_d,"w3"=w3_d,"sdata"=sddata)
  return(res)
}
...
fmboxplot_i <- function(X, t, rangeval, nbasis, norder) {
  ...
  results<-ind(X, t, rangeval, nbasis, norder)
  indexes<-results$ind
  sdata<-results$sd2data
  n<-nrow(indexes)
  hypo<-sort(indexes$d2dtaMHI)
  epi<-sort(indexes$d2dtaMEI)
  #75% con mayor Hypograph
  q_h<-hypo[round(n/4)+1]
  sorted_h <- as.data.frame(subset(sdata,indexes$d2dtaMHI>=q_h))
  #75% con menor Epigraph
  q_e<-epi[round(n*3/4)+1]
  sorted_e <- as.data.frame(subset(sdata,indexes$d2dtaMEI<=q_e))
  #Bucle para crear q1 y q3
  q1<-rep(0,length(t))
