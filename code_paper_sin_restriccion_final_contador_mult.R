...
sim_segun <- function(clus2=22){
  #100 rows (50 outliers in the first 50 rows: group 1)
  #3 dimensions
  
  if(clus2==22)
    #data_model <- multimodel1(n_outliers = 50)
    data_model <- multimodel1(n_outliers = 50)
  else if(clus2==23)
    data_model <- multimodel2(n_outliers = 50)
  else if(clus2==24)
    data_model <- multimodel3(n_outliers = 50)
  else if(clus2==25)
    data_model <- multimodel4(n_outliers = 50)
  else if(clus2==26)
    data_model <- multimodel5(n_outliers = 50)
  else if(clus2==27)
    data_model <- multimodel6(n_outliers = 50)
  else if(clus2==28)
    data_model <- multimodel7(n_outliers = 50)
  else if(clus2==29)
    data_model <- multimodel8(n_outliers = 50)
  else if(clus2==30)
    data_model <- multimodel9(n_outliers = 50)
  
  K<-dim(data_model[[1]]@X)[1]
  P<-dim(data_model[[1]]@X)[2]
  
  data <- array(rep(NaN,K*P),dim=c(K,P,3)) # datos finales
  data[,,1] <- data_model[[1]]@X
  data[,,2] <- data_model[[2]]@X
  data[,,3] <- data_model[[3]]@X
  return(data)
}

sim3<- function(n=10,t=seq(0,1,length=15)){
  X <- matrix(0,n,length(t))
  noise <- mvrnorm(n=10,mu=rep(0, length(t)), Sigma = 0.3*diag(length(t)))
  
  X1 <- X
  for (i in 1:n){
    for (j in 1:length(t)){
      X1[i,j] <- t[j]*t[j]+noise[i,j]
    }
  }
  ...
  for (i in 1:n){
    for (j in 1:length(t)){
      X6[i,j] <- t[j]-1+noise[i,j]
    }
  }
  
  Xres <- rbind(X1, X2, X3, X4, X5, X6)
  return(Xres)
}

sim4 <- function(n=21,t=seq(-6,6,length=150),mean_a=0,sd_a=1.25){
  set.seed(1)
  X <- matrix(0,n,length(t))
  # noise <- mvrnorm(n=10,mu=rep(0, length(t)), Sigma = 0.3*diag(length(t)))
  z1 <- rnorm(n, mean=1, sd=0.05)
  z2 <- rnorm(n, mean=1.5, sd=0.1)
  a <- rnorm(n, mean_a, sd_a)
  X1 <- X
  for (i in 1:n){
    for (j in 1:length(t)){
      X1[i,j] <- z1[i]*exp(-(t[j]-a[i])^2/2)
    }
  }
...
datos_sim <- function(nsim){
  ...
  else if(nsim>=22 & nsim<=30)
    data <- sim_segun(clus2=nsim)
  # New univariate simulations (revision paper)
  else if(nsim==31)
    data<-sim3()
  else if(nsim==32)
    data<-sim4()
